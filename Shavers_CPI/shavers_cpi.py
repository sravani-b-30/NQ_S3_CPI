...
def convert_weight_to_kg(weight_str):
    ...
    #     return value
    # return None
    # Regex to capture a number and a unit that can be pounds, kilograms, ounces, or grams.
    pattern = re.compile(r'(\d+\.?\d*)\s*(pounds?|lbs?|kilograms?|kgs?|ounces?|oz|grams?|gms?|g)\b')
    match = pattern.search(weight_str)
    if match:
        value, unit = match.groups()
        value = float(value)
        if unit.startswith('pound') or unit in ['lb', 'lbs']:
            # Convert pounds to kg
            return value * 0.453592
        elif unit.startswith('ounce') or unit in ['oz']:
            # Convert ounces to kg (1 oz = 0.0283495 kg)
            return value * 0.0283495
        elif unit.startswith('gram') or unit in ['gms', 'g']:
            # Convert grams to kg (1 g = 0.001 kg)
            return value * 0.001
        elif unit.startswith('kilogram') or unit in ['kg', 'kgs']:
...
def calculate_similarity(details1, details2, title1, title2, desc1, desc2):
    ...
    for key in details1.keys():
        if key in details2:
            ...
            if 'weight' or 'item weight' in key.lower():
...
def load_and_preprocess_data(s3_folder):
    ...
    return asin_keyword_df, keyword_id_df, merged_data_df, unique_brands
...
def get_data(s3_folder, refresh=False):
    ...
    asin_keyword_df, keyword_id_df, merged_data_df, unique_brands = load_and_preprocess_data(
        s3_folder
    )
...
if st.button("Refresh Data"):
    with st.spinner("Refreshing data..."):
        # Force a refresh
        asin_keyword_df, keyword_id_df, merged_data_df, unique_brands = get_data(
    ...
    if st.session_state.get('recompute', False) or st.button('Run Analysis Again'):
        ...
        # Save the combined competitor details DataFrame to S3 and generate presigned URL
            if not combined_competitor_df.empty:
                ...
                # Define the S3 key
                s3_key = f"SHAVERS/combined_competitors_{asin}_{start_date}_{end_date}.csv"
