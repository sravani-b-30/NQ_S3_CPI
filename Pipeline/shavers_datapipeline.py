...
def fetch_keyword_ids(keywords):
    """
    Fetches keyword IDs for given keywords from the database.
    """

    logger.info(f"Successfully loaded the listed keywords into keyword_id fetching function : {keywords}")
...
def align_and_combine_serp_data(df_product_data):
    """
    ...
    for col in required_columns:
        if col not in df_product_data.columns:
            df_product_data[col] = None

    df_product_data = df_product_data[required_columns]
   
    

    df_product_data['date'] = pd.to_datetime(df_product_data['date']).dt.date
    df_product_data = df_product_data.sort_values(by=['asin', 'date'], ascending=[True, True])
    deduplicated_data = df_product_data.groupby(['asin', 'date'], as_index=False).last()
...
def fetch_latest_napqueen_file(bucket_name, brand, prefix="SHAVERS", file_extension=".csv"):
    """
...
def scrapper_handler(df, bucket_name, brand, file_name="SHAVERS.csv", num_workers=15):
...
def fetch_latest_file_from_s3(bucket_name, prefix="SHAVERS/merged_data_", file_extension=".csv"):
    """
...
if __name__ == '__main__':
    ...
    df_keywords = fetch_keyword_ids(keywords)
    df_serp = fetch_serp_data(df_keywords)

    df_product_data = fetch_and_merge_product_data(df_serp)


    final_combined_data = align_and_combine_serp_data(df_product_data)
