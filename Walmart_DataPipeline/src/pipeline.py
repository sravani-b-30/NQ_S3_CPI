...
def product_details_merge(final_df, product_details_df):
...
def main():
    ...
    nq_df = fetch_napqueen_products(marketplace="Walmart")
    ...
    final_df = clean_napqueen_data(nq_merged_df, merged_df)
    
    # Process and Update Product Details
    logger.info("Processing and Updating Product Details...")
    product_details_df = process_and_update_product_details(final_df)

    # Merge Product Details with Final Dataset
    final_merged_df = product_details_merge(final_df, product_details_df)
