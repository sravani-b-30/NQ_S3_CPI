...
def fetch_serp_data(updated_df):
    ...
    logger.info("Processed SERP data.")
    return filtered_df
...
def fetch_and_merge_product_data(df):
    ...
    try:
        ...
        logger.info("Fetched and merged product data.")
        return merged_df
    except Exception as e:
...
def fetch_and_enrich_price_data_by_date_range(start_date, end_date):
    ...
    try:
        ...
        cursor = conn.cursor()
        query = """
        SELECT date, product_id, asin, product_title, brand, price, availability
        ...
        for current_date in unique_dates:
            daily_data = price_data[price_data['date'] == current_date]
            product_ids = daily_data['product_id'].unique().tolist()
            ...
            WHERE product_id = ANY(%s);
            """
            cursor.execute(query_keyword_id, (product_ids,))
            keyword_id_data = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])

            keyword_ids = keyword_id_data['keyword_id'].unique().tolist()
            query_keyword = """
            SELECT keyword_id, keyword
            FROM serp.keywords
            WHERE keyword_id = ANY(%s);
            """
            cursor.execute(query_keyword, (keyword_ids,))
            keyword_data = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])

            daily_enriched = pd.merge(daily_data, keyword_id_data, on='product_id', how='left')
            daily_enriched = pd.merge(daily_enriched, keyword_data, on='keyword_id', how='left')
            enriched_data_list.append(daily_enriched)

        cursor.close()
        conn.close()

        enriched_data = pd.concat(enriched_data_list, ignore_index=True)
        logger.info("Fetched and enriched SP API price data.")
        return enriched_data
    except Exception as e:
        logger.error(f"Error fetching and enriching price data: {e}")
        raise
...
def align_and_combine_serp_and_sp_api_data(serp_data, sp_api_data):
    ...
    """
    required_columns = ['asin', 'product_id', 'keyword_id', 'title', 'brand', 'price', 'keyword', 'date']
    ...
    logger.info("Combined and deduplicated SERP and SP API data.")
